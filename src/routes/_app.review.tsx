...
function ReviewPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      let q = supabase.from("monthly_review").select("entity_id,period_month,status,remarks").order("period_month", { ascending: false });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card><CardContent className="p-5"><div className="text-xs text-muted-foreground">Total reviews</div><div className="text-2xl font-semibold">{rows.length}</div></CardContent></Card>
        <Card><CardContent className="p-5"><div className="text-xs text-muted-foreground">Passes</div><div className="text-2xl font-semibold text-aiesec-green">{passes}</div></CardContent></Card>
        <Card><CardContent className="p-5"><div className="text-xs text-muted-foreground">Fails</div><div className="text-2xl font-semibold text-aiesec-red">{fails}</div></CardContent></Card>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          <Table>
            ...
            <TableBody>
              {rows.length === 0 && <TableRow><TableCell colSpan={4} className="py-6 text-center text-muted-foreground">No data</TableCell></TableRow>}
              {rows.map((r, i) => (
                <TableRow key={i}>
                  <TableCell>{entityName(r.entity_id)}</TableCell>
                  <TableCell>{format(parseISO(r.period_month), "MMM yyyy")}</TableCell>
                  <TableCell>
                    {r.status === "pass" ? <Badge className="bg-aiesec-green/15 text-aiesec-green hover:bg-aiesec-green/15">Pass</Badge>
                      : r.status === "fail" ? <Badge className="bg-aiesec-red/15 text-aiesec-red hover:bg-aiesec-red/15">Fail</Badge>
