...
function AuditPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      let q = supabase.from("audit_scores").select("entity_id,period_month,quarter,score,max_score,remarks").order("period_month", { ascending: false });
  ...
  const exportCSV = () => {
    ...
    const body = rows.map((r) => [entityName(r.entity_id), r.period_month, r.quarter ?? "", r.score ?? "", r.max_score ?? "", `"${(r.remarks ?? "").replace(/"/g, '""')}"`].join(",")).join("\n");
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <Table>
            ...
            <TableBody>
              {quarterly.length === 0 && <TableRow><TableCell colSpan={4} className="py-6 text-center text-muted-foreground">No data</TableCell></TableRow>}
              {quarterly.map((q) => {
                ...
                return (
                  <TableRow key={q.quarter}>
                    ...
                    <TableCell><ScoreBadge pct={pct} /></TableCell>
                  </TableRow>
                );
              })}
            </TableBody>
      ...
      <Card>
        <CardHeader><CardTitle className="text-base">Monthly audit breakdown</CardTitle></CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
              ...
              {rows.map((r, i) => {
                ...
                return (
                  <TableRow key={i}>
                    ...
                    <TableCell className="text-right">{r.score?.toFixed(0)}/{r.max_score?.toFixed(0)}</TableCell>
                    <TableCell><ScoreBadge pct={pct} /></TableCell>
                    <TableCell className="max-w-xs truncate text-muted-foreground">{r.remarks}</TableCell>
                  </TableRow>
                );
              })}
            </TableBody>
...
function ScoreBadge({ pct }: { pct: number }) {
  if (pct >= 80) return <Badge className="bg-aiesec-green/15 text-aiesec-green hover:bg-aiesec-green/15">Excellent</Badge>;
  if (pct >= 60) return <Badge className="bg-aiesec-orange/15 text-aiesec-orange hover:bg-aiesec-orange/15">Acceptable</Badge>;
  return <Badge className="bg-aiesec-red/15 text-aiesec-red hover:bg-aiesec-red/15">Needs attention</Badge>;
}
